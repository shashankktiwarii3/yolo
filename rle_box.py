...
# ----------------------------------------------------------------------------
# CONFIRMED structure of the installed YOLO26 Detect head (runtime
# inspection): Detect with end2end=True and two ModuleDict branches
#   head.one2many = {"box_head": <per-level>, "cls_head": <per-level>}
#   head.one2one  = {"box_head": <per-level>, "cls_head": <per-level>}
# Add a mirrored "sig_head" entry to BOTH dicts and change the per-level
...
#               sig = copy.deepcopy(branch["box_head"])   # clone box topology
#               for lvl in sig:                            # per-level ModuleList
#                   final = lvl[-1]                        # last conv -> 4 sigma ch
#                   lvl[-1] = nn.Conv2d(final.in_channels, 4,
#                                       final.kernel_size, final.stride,
#                                       final.padding if hasattr(final, "padding") else 0)
#                   nn.init.constant_(lvl[-1].bias, -2.0)  # sigma_0 ~ 0.12: no cold-start spike
#               branch["sig_head"] = sig
#
#       # In the branch forward (copy the installed body, one line per level):
#       #   x[i] = cat((box_head[i](f), sig_head[i](f), cls_head[i](f)), 1)
#       # At decode: sigma = raw_sig.sigmoid()  ->  feed sigma_rank_scores()
#       #            just before the topk(300) selection in postprocess().
#
# Register in ultralytics.nn.tasks parse_model (or, since you run a cloned
# repo, edit head.py directly) and duplicate yolo26.yaml -> yolo26-tiny.yaml
# swapping the head class. Weight surgery: pretrained box_head/cls_head load
# untouched by name; sig_head starts fresh with the -2.0 bias init above.
# Check MuSGD's param-group filter routes the new params (sig convs + flow
# linears -> matrix/Muon group; their biases -> 1D SGD group).
#
